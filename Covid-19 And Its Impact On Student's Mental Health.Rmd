---
title: "Covid-19 And Its Impact On Student's Mental Health"
...
data <- read_csv("C:\\Users\\shami\\OneDrive\\Documents\\DSP 2\\COVID-19 and Its Impact On Students Mental Health.csv") %>%
clean_names() %>%
...
"MealsPerDay", "WeightChange", "HealthIssue", "StressBuster","TimeUtilized", "FeelCloser", "MissMost")
```
```{r}
# Create age range categories
data <- data %>%
mutate(AgeRange = cut(Age, breaks = c(5, 10, 15, 20, 25, 30, 35, 40, 45, 50, 55, 60),
labels = c("5-10", "11-15", "16-20", "21-25", "26-30", "31-35", "36-40", "41-45",
"46-50", "51-55", "56-60")))
...
menuItem("Average Time Spent (Activities)", tabName = "time_activities", icon =
icon("chart-bar")),
menuItem("Sleep Impact", tabName = "sleep_impact", icon = icon("bed")),
menuItem("Rating Vs. Time Spent Online", tabName = "online_class", icon =
icon("laptop")),
...
menuItem("Time Spent (Online Class)", tabName = "scatter_plot", icon = icon("chart-line")),
# Changed icon to chart-line
...
p("This dashboard provides insights into the impact of COVID-19 on students' mental
health."),
...
selectInput("region", "Select Region", choices = c("All Regions", unique(data$Region)),
selected = "All Regions"),
selectInput("activity", "Select Activity", choices = c("Online Class" = "OnlineClassHours",
"Self Study" = "SelfStudyHours", "Fitness" = "FitnessHours", "Sleep" = "SleepHours", "Social
Media" = "SocialMediaHours", "TV" = "TVHours"), selected = "OnlineClassHours"),
sliderInput("slider", "Age Range", min = min(data$Age, na.rm = TRUE), max =
max(data$Age, na.rm = TRUE), value = c(min(data$Age, na.rm = TRUE), max(data$Age, na.rm
= TRUE)))
),
tabItem(
tabName = "social_media",
plotOutput("plot_social_media"),
selectInput("region_social", "Select Region", choices = c("All Regions",
unique(data$Region)), selected = "All Regions"),
selectInput("social_media", "Select Social Media", choices = c("All",
unique(data$SocialMediaPlatform)), selected = "All")
),
tabItem(
tabName = "good_bad",
...
labs(title = "Average Time Spent on Different Activities and Gadgets", x = "Activity", y =
"Average Hours") +
theme_minimal() +
...
labs(title = "Impact of Social Media and TV on Sleep Hours", x = "Time Spent on Social
Media and TV (Hours)", y = "Sleep Hours") +
theme_minimal()
...
p <- ggplot(data, aes(x = factor(OnlineClassRating), y = OnlineClassHours, fill =
factor(OnlineClassRating))) +
geom_boxplot() +
labs(title = "Online Class Rating vs. Time Spent Online", x = "Online Class Rating", y =
...
labs(title = "Age Distribution for Health Issues (Yes)", x = "Age", y = "Count", fill = "Health
Issue") +
theme_minimal()
...
labs(title = "Average Change in Weight by Health Issues", x = "Health Issue", y = "Average
Weight Change") +
theme_minimal() +
...
scatter_plot(data = data, activity = input$activity, min_age = input$slider[1], max_age =
input$slider[2], region = input$region)
})
...
data %>% filter(Region == input$region_social, SocialMediaPlatform ==
input$social_media)
}
})
...
pivot_longer(cols = c("MeanGood", "MeanBad"), names_to = "Type", values_to =
"MeanHours")
